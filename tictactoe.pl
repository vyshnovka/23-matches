play :-
    ...
    writeln("Choose a number to make a move:"),
...
playfrom(Board) :- 
    ...
    writeln("Game over! :(").
playfrom(Board) :- 
    read(N), xmove(Board, N, Newboard), 
    table(Newboard), omove(Newboard, Newnewboard), 
    table(Newnewboard), playfrom(Newnewboard).
